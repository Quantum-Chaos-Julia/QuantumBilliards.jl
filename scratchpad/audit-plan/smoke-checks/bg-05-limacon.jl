...
# LimaconBilliard: Z2 symmetry (XAxisReflection only), PolarDomain fundamental
# domain built from a LimaconSegment + closing LineSegment along the axis.
a = 0.3
billiard = LimaconBilliard(a)
try
    closed, winding_ok, ncurves = check_full_boundary(billiard; label="LimaconBilliard")
    @assert ncurves == 2 "expected 2 image arcs (upper+lower half), got $ncurves"
    @assert closed "full_boundary not closed/continuous for LimaconBilliard"
    @assert winding_ok "full_boundary self-overlaps or has gaps for LimaconBilliard"
    println("PASS: LimaconBilliard full_boundary reconstruction OK")
catch e
    println("CONFIRMED CRASH: full_boundary(LimaconBilliard) errored with: ", sprint(showerror, e))
    println("Root cause: no _apply_symmetry_to_curve method exists for LimaconSegment in fullboundary.jl")
    println("(LineSegment/CircleSegment/FourierCoeffPolarSegment/PolarSegment all have one; LimaconSegment does not.)")
end
