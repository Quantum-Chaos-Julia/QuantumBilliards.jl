...
# EllipseBilliard: D2 symmetry, quadrant fundamental domain built from a
# function-based PolarSegment (not FourierCoeffPolarSegment) -- exercises the
# _apply_symmetry_to_curve/_reverse_curve method for L<:PolarSegment.
a, b = 2.0, 1.0
billiard = EllipseBilliard(a, b)
closed, winding_ok, ncurves = check_full_boundary(billiard; label="EllipseBilliard")
@assert ncurves == 4 "expected 4 image arcs (quadrants), got $ncurves"
@assert closed "full_boundary not closed/continuous for EllipseBilliard"
@assert winding_ok "full_boundary self-overlaps or has gaps for EllipseBilliard"

println("PASS: EllipseBilliard full_boundary reconstruction OK")
