...
# RectangleBilliard: D2 symmetry, first-quadrant fundamental domain.
a, b = 2.0, 1.0
billiard = RectangleBilliard(a, b)
closed, winding_ok, ncurves = check_full_boundary(billiard; label="RectangleBilliard")
@assert ncurves == 8 "expected 8 image edges (2 physical edges x 4 quadrant copies), got $ncurves"
@assert closed "full_boundary not closed/continuous for RectangleBilliard"
@assert winding_ok "full_boundary self-overlaps or has gaps for RectangleBilliard"

total_len = sum(c.length for c in full_boundary(billiard))
expected = 2*(2a) + 2*(2b)
println("[RectangleBilliard] total perimeter = $total_len, expected $expected")
@assert isapprox(total_len, expected; rtol=1e-10) "perimeter mismatch: $total_len vs $expected"

println("PASS: RectangleBilliard full_boundary reconstruction OK")
